...
local function getPlayerMoney( player) -- 获得玩家积分
	...
	local Q = AuthDBQuery("select moneys from account_money where id = " .. uid)
	local moneys = 0	 
	if Q then
		repeat
			moneys  = Q:GetUInt32(0) 
		until not Q:NextRow()		 
		AuthDBExecute("update account_money set moneys = ".. moneys  .." where id = ".. uid .. "") 
	else	 
		AuthDBExecute("insert into account_money values(".. uid ..",".. moneys .. ")") 
	end
	return moneys
end
...
function ST.SummonENPC(player)--召唤幻化
...
local function ResetPlayer(player, flag, text)
	...
	--player:LogoutPlayer(true) 禁用，在角色选择界面服务器会当机
end
...
local Stone={
	...
	WSkillsToMax=function(player)--技能熟练度
        if(player:HasItem( 41001, 20 ))then  -- 需求荣誉勋章
		    player:AdvanceSkillsToMax()
            player:RemoveItem(41001, 20 )
		    player:SendBroadcastMessage("当前技能熟练度已经达到最大值")
		else
			player:SendAreaTriggerMessage("需要艾泽拉斯荣誉勋章20。")
			player:SendBroadcastMessage("需要艾泽拉斯荣誉勋章20。")
		end                                       
	end,
...
local Menu={
	[MMENU]={--主菜单
		...
		--{FUNC, "重置所有副本进度", 	Stone.UnBind,	GOSSIP_ICON_INTERACT_1, false,"是否重置所有副本进度 ?"},
		--{MENU, "职业技能训练师",MMENU+0x20,		GOSSIP_ICON_BATTLE},
		--{MENU, "专业技能训练师",MMENU+0x30,		GOSSIP_ICON_BATTLE},
		--{FUNC, "强制脱离战斗", 	Stone.OutCombat,GOSSIP_ICON_BATTLE},
		--{TP, "商业中心广场", 0, -13229.116211, 225.751312, 32.686565, 1.133195},
	},
	...
	[TPMENU]={--传送菜单
		...
		{MENU, "五人地下城",	TPMENU+0x60,GOSSIP_ICON_BATTLE},
	...
	[TPMENU+0x130]={--卡利姆多 2
		...
		{TP, "塔纳利斯", 1, -6940.91, -3725.7, 48.9381, 3.11174},
	...
	[TPMENU+0x60]={--五人地下城
		{TP, "[14]怒焰裂谷",  1, 1811.78, -4410.5, -18.4704, 5.20165},
		{TP, "[19]死亡矿井", 0, -11209.6, 1666.54, 24.6974, 1.42053},
		{TP, "[22]哀嚎洞穴", 1, -731.607, -2218.39, 17.0281, 2.78486},
		{TP, "[24]影牙城堡", 0, -234.675, 1561.63, 76.8921, 1.24031},
		{TP, "[26]黑暗深渊", 1, 4249.99, 740.102, -25.671, 1.34062},
		{TP, "[27]暴风城监狱", 0, -8799.15, 832.718, 97.6348, 6.04085},
		{TP, "[31]剃刀沼泽", 1, -4470.28, -1677.77, 81.3925, 1.16302}, 
		{TP, "[33]诺莫瑞根",0, -5163.54, 925.423, 257.181, 1.57423},
		{TP, "[40]血色修道院", 0, 2873.15, -764.523, 160.332, 5.10447},
		{TP, "[42]剃刀高地",  1, -4657.3, -2519.35, 81.0529, 4.54808}, 
		{MENU, "下一页", TPMENU+0x160,GOSSIP_ICON_CHAT},
	},	
		
	[TPMENU+0x160]={--五人地下城 2
		{TP, "[45]奥达曼",0, -6071.37, -2955.16, 209.782, 0.015708},
		{TP, "[46]祖尔法拉克", 1, -6801.19, -2893.02, 9.00388, 0.158639},
		{TP, "[49]玛拉顿", 1, -1421.42, 2907.83, 137.415, 1.70718},
		{TP, "[53]沉没的神庙", 0, -10177.9, -3994.9, -111.239, 6.01885},
		{TP, "[57]黑石深渊", 0, -7179.34, -921.212, 165.821, 5.09599},
		{TP, "[60]通灵学院", 0, 1269.64, -2556.21, 93.6088, 0.620623}, 
		{TP, "[60]斯坦索姆", 0, 3352.92, -3379.03, 144.782, 6.25978}, 
		{TP, "[60]厄运之槌", 1, -3520.14, 1119.38, 161.025, 4.70454},
		{TP, "[60]黑石塔", 0, -7527.05, -1226.77, 285.732, 5.29626},	
	},
...
-- RegisterCreatureGossipEvent(itemEntry, 1,  Stone.ShowGossip)
-- RegisterCreatureGossipEvent(itemEntry, 2, Stone.SelectGossip)