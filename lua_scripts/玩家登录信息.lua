...
local function PlayerFirstLogin(event, player)--玩家首次登录
	SendWorldMessage("|cFFFF0000[系统]欢迎|r"..GetPlayerInfo(player).." |cFFFF0000首次进入灵境魔兽世界。|r")
	--玩家首次登录 初始化积分表
...
local function PlayerLogin(event, player)--玩家登录
	SendWorldMessage("|cFFFF0000[系统]|r欢迎"..GetPlayerInfo(player).." 上线，祝游戏愉快！")
	print("Player is Login. GUID:"..player:GetGUIDLow())
end

--local function PlayerLogout(event, player)--玩家登出
	--SendWorldMessage("|cFFFF0000[系统]|r"..GetPlayerInfo(player).." 下线,欢迎下次再来。")
